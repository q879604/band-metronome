...
| 两种引擎 | **逐拍震动** `vibrator.vibrate({mode})`：全机型可用，**手环 9 Pro 只有这一条路**（系统长/短两档）<br>**原生任务** `vibrator.start()`：官方支持明细里只有 Xiaomi Watch S5 实现，手环 9/9 Pro 未实现——选它会自动检测并回退到逐拍 |
...
而且 **手环 9 Pro 连 `getSystemDefaultMode()` 都没实现**（见下表），设置页会显示「本机不支持读取」。

### 官方支持明细（Vela 官方文档 · 振动 vibrator）

| 接口 | 手环 9 / 9 Pro | 支持机型 |
| --- | --- | --- |
| `vibrator.vibrate({mode})` | ✅ 支持 | 手环 8 Pro / 9 / 9 Pro / 10、Redmi Watch 4、Watch H1 / S3 / S4 / S5、REDMI Watch 5 / 6、S1 Pro |
| `vibrator.start()` / `stop()` / `getSystemDefaultMode()` | ❌ 不支持 | 仅 **Xiaomi Watch S5** |

### 息屏的实话

`vibrator.vibrate` 只能从 JS 调，而快应用退到后台 / 息屏后 JS 计时器会被系统挂起 → **息屏后节拍基本不会继续**
（`start()` 那条路本来能绕开 JS，但手环 9 Pro 没实现）。所以默认方案是「**最低亮度 + 常亮**」：
屏幕不熄但暗到几乎看不见，节拍照跑；连点 5 次屏幕恢复原亮度与自动亮度。
